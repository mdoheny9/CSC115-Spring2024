public class A5Tester {
	...
	public static void testCountSongs() {
		...
		// new: 
		LinkedSongList listA = new LinkedSongList();
		LinkedSongList listB = new LinkedSongList();
		LinkedSongList listC = new LinkedSongList();
		LinkedSongList listD = new LinkedSongList();
		...
		// if there are multiple of the same song
		Song[] songsWithPlural = {s1, s2, s1, s4};
		listA.buildFromArray(songsWithPlural);

		result = listA.countSongs();
		expected = 4;
		displayResults(result==expected, "countSongs with two identical songs");

		// if the list is empty
		Song[] songsEmpty = {};
		listB.buildFromArray(songsEmpty);

		result = listB.countSongs();
		expected = 0;
		displayResults(result==expected, "countSongs with empty list");
	...
	public static void testTotalDuration() {
		...
		// list is empty
		Song[] songsEmpty = {};
		listA.buildFromArray(songsEmpty);

		result = listA.totalDuration();
		expected = 0;
		displayResults(result==expected, "totalDuration in empty list");

		// invalid song ?
		Song sInvalid = new Song("FANCY", "Twice", -1);
		Song[] songsInvalid = {sInvalid};
		listB.buildFromArray(songsInvalid);

		result = listB.totalDuration();
		expected = -1;
		displayResults(result==expected, "totalDuration in invalid list");
	...
	public static void testCountSongsByArtist() {
		...
		// no songs by artist in list
		artistToFind = new String("NewJeans");
		result = list2.countSongsByArtist(artistToFind);
		expected = 0;
		displayResults(result==expected, "songs by "+artistToFind+" in list2");

		// every song is by artist
		artistToFind = new String("TWICE");
		Song sA = new Song("alchohol free", "TWICE", 260);
		Song sB = new Song("fancy", "TWICE", 180);
		Song sC = new Song("talk that talk", "TWICE", 120);
		Song[] songsByTwice = {sA, sB, sC};
		listA.buildFromArray(songsByTwice);

		result = listA.countSongsByArtist(artistToFind);
		expected = 3;
		displayResults(result==expected, "songs by "+artistToFind+" in listA");
