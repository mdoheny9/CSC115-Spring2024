...
public class Lab2 {
	...
	public static String[] getClasslist(Student[] students) {
		...
		for (int i = 0; i < students.length; i++) {
			String eachID = students[i].getSID();
			sIDs[i] = eachID;
		}
		return sIDs;
	}
	...
	public static double getClassAverage(Student[] students) {
		...
		for (int i = 0; i < students.length; i++) {
			int eachGrade = students[i].getGrade();
	...
	public static Student[] registerStudent(Student[] students, Student s) {
		Student[] newClassList;
		newClassList = new Student[students.length + 1];
		for (int i = 0; i < students.length; i++) {
			newClassList[i] = students[i];
		}
		newClassList[students.length] = s;
