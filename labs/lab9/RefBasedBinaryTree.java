...
public class RefBasedBinaryTree implements BinaryTree {
    ...
    private int sumRec(TreeNode cur) {
        if (cur==null) {
            ...
            return cur.getValue() + sumRec(cur.left) + sumRec(cur.right);
        }
    }
    ...
    private int getMaxRec(TreeNode cur, int max) {
        if (cur == null) {
            return max; 
        } else {
            if (cur.getValue() > max) {
            ...
            max = getMaxRec(cur.getLeft(), max);
            max = getMaxRec(cur.getRight(), max);
            return max;
        }
    }
